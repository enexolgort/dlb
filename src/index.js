...
function evaluatePrefix(expression, mem) {
    ...
    function evalRecursive() {
        ...
        if (token === "premier") {
            const list = evalRecursive();
            if (!Array.isArray(list))
                throw new Error(`"premier" expects a list`);
            return list[0];
        }
        if (token === "dernier") {
            const list = evalRecursive();
            if (!Array.isArray(list))
                throw new Error(`"dernier" expects a list`);
            return list[list.length - 1];
        }
        if (token === "taille") {
            const list = evalRecursive();
            if (!Array.isArray(list))
                throw new Error(`"taille" expects a list`);
            return list.length;
        }
        if (token === "car") {
            const code = evalRecursive();
            if (typeof code !== "number")
                throw new Error(`"car" expects a number`);
            return String.fromCharCode(code);
        }
        if (token === "de") {
            const list = evalRecursive();
            const index = evalRecursive();
            if (!Array.isArray(list))
                throw new Error(`"a" expects a list`);
            if (index < 0 || index >= list.length)
                throw new Error(`Index ${index} out of bounds (list length: ${list.length}) at ip=${mem.ip ?? "?"}`);
            return list[index];
        }
        // Binary operators
        if ([
...
function tokenize(input) {
    const regex = /\s*('(?:[^'\\]|\\.)*'|"(?:[^"\\]|\\.)*"|>=|<=|!=|AND|OR|NOT|premier|dernier|taille|car|[\[\]()+\-*\/<>=,]|[A-Za-z_][A-Za-z0-9_]*|\d+)\s*/g;
    ...
    for (const token of reversed) {
        if (token === "(") {
            ...
            while (operators.length > 0) {
                ...
                if (topPrecedence === undefined || tokenPrecedence === undefined)
                    break;
...
function getList(mem, name) {
    if (!Array.isArray(mem[name]))
        throw new Error(`"${name}" is not a list`);
    return mem[name];
}
function resolveTarget(target, labels, mem) {
...
function main() {
    ...
    while (mem.lineIndex < lines.length) {
        const line = lines[mem.lineIndex];
        if (line == undefined)
            break;
        if (line.match(/jai\s+(\w+)\s*=\s*programme\s*'(.+)';/)) {
            const [, name, code] = line.match(/jai\s+(\w+)\s*=\s*programme\s*'(.+)';/) ?? [];
            mem[name] = code.split("");
            mem.lineIndex += 1;
        }
        else if (line.match(/jai\s+(\w+)\s+de\s+(.+)\s*=\s*(.+);/)) {
            const [, listName, indexExpr, valueExpr] = line.match(/jai\s+(\w+)\s+de\s+(.+)\s*=\s*(.+);/) ?? [];
